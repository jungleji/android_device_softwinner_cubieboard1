...
static int find_name_map(struct sunxi_audio_device *adev, char * in, char * out)
{
	...
	{
            ALOGD("index = %d, audio_name_map[index].name_linux = %s", index, audio_name_map[index].name_linux);
...
static int do_init_audio_card(struct sunxi_audio_device *adev, int card)
{
	...
	if(ret == 0)
	{
		...
		ALOGD("find name map, snd_id = %s", snd_id);
                         ...
                         size_t bytes)
{
	...
	if(ret!=0)
	{
		ALOGE("##############out_write()  Warning:write fail#######################");
	}
	}
                     ...
                     hw_device_t** device)
{
    ...
    adev->mixer_ctls.audio_speaker_out = mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_SPEAKER_OUT);
