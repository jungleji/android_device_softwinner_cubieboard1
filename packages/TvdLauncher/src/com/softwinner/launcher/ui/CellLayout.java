...
public class CellLayout extends ViewGroup implements View.OnLongClickListener{
    ...
    public class WidgetMenu extends AlertDialog implements DialogInterface.OnClickListener{
        
        protected static final int BUTTON_ADD_WIDGET = BUTTON_POSITIVE;
        protected static final int BUTTON_DEL_WIDGET = BUTTON_NEUTRAL;
        
        protected WidgetMenu(Context context) {
            super(context);
            setButton(BUTTON_ADD_WIDGET, context.getText(R.string.add_favorites_app), this);
            setButton(BUTTON_DEL_WIDGET, context.getText(R.string.uninstall_app), this);
        }

        @Override
        public void onClick(DialogInterface dialog, int button) {
            switch(button){
                case BUTTON_ADD_WIDGET:
                    
                    break;
                case BUTTON_DEL_WIDGET:
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        Log.d("Cell","w=" + mCellWidth + " h=" + mCellHeight);
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    private boolean mDrawCell = false;
    private RectF mDrawCellRect = new RectF();
    private Paint mPaint= new Paint();
    public void drawCellRect(int x, int y, int spanX,int spanY){
        cellToRect(x,y,spanX,spanY,mDrawCellRect);
        mDrawCell = true;
        invalidate();
    }
    ...
    public void removeViewFromCell(int x, int y){
        for(int i=0;i<getChildCount();i++){
            View v = getChildAt(i);
            LayoutParams cellParams = (LayoutParams) v.getLayoutParams();
            Log.d("", "cy=" + cellParams.cellX +" cx="+ cellParams.cellY + " x="+x+" y="+y);
            if( cellParams.cellX == x && cellParams.cellY == y){
                removeView(v);
            }            
        }
        invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if(mDrawCell){
            mPaint.setColor(mContext.getResources().getColor(R.color.grayColor));
            canvas.drawRect(mDrawCellRect, mPaint);
        }
    }
    ...
    protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        if(gainFocus && getChildCount() > 0 ){
            getChildAt(0).requestFocus(direction);
        }
    }
